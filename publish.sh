...
command -v node >/dev/null || { echo "Node.js가 필요해."; exit 1; }
command -v python3 >/dev/null || { echo "Python 3가 필요해."; exit 1; }
...
UNAME="$(gh api user --jq '.name // .login')"
...
git config user.email "${UID_NUM}+${OWNER}@users.noreply.github.com"
...
if gh repo view "$REPO" >/dev/null 2>&1; then
  ...
  gh repo create "$REPO" \
    --public \
    --description "수령길-컴맹 retro responsive memo app" >/dev/null
  git remote add origin "$URL"
fi
...
echo
echo "새 버전 확인: https://${OWNER}.github.io/${REPO_NAME}/v094.html"

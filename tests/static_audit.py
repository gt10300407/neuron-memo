...
VERSION_HTML_PATH = ROOT / "v095.html"
JS_PATH = ROOT / "app-v095.js"
CSS_PATH = ROOT / "app-v095.css"
...
assert '<title>수령길-컴맹 v0.9.5</title>' in html
...
assert parser.scripts[0].get("src") == "app-v095.js"
...
for relative in ("app-v095.css", "app-v095.js", "icons/icon-192.png", "icons/icon-512.png"):
...
old_pages = [path.name for path in ROOT.glob("v*.html") if path.name != "v095.html"]
...
# v0.9.5 mobile graph regression
...
# v0.9.5 pinch regression
assert "anchorX:middle.x" in js
assert "anchorY:middle.y" in js
assert "gt.x=pinch.anchorX-pinch.worldX*scale" in js
assert "gt.y=pinch.anchorY-pinch.worldY*scale" in js
assert "const center=(graphBounds.minX+graphBounds.maxX)/2" not in js
